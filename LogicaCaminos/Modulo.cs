...
{
    public class Modulo: AccesoDatos
    {
        ...
        public Modulo(int id_empresa = 0)
        {
            id_modulo = 0;
            id_proyecto = 0;
            NombreModulo = "";
        ...
        private int Modificar_()
        {
            ...
            dtProyecto = Obtener(id_modulo);

            if (id_modulo != 0)
            {
                sec = (int)EjecutarEscalar("Select sec from modulo where id_modulo = " + id_modulo + " ");
            }

            lStb.Append("update modulo set ");
            ...
            lStb.Append(" where id_modulo = " + id_modulo + " and sec = " + sec + " ");
        ...
        private int Registro()
        {
            ...
            if (id_modulo == 0)
            {
                id_modulo = (int)EjecutarEscalar("Select isnull(max(id_modulo),0) + 1 from modulo ");
                sec = 1;
            }
            else
            {
                sec = (int)EjecutarEscalar("Select max(sec) from modulo where id_modulo = " + id_modulo + " ") + 1;
            }

            lStb.Append("INSERT INTO modulo (id_modulo, Nombremodulo, id_proyecto, id_empresa, sec, estado, fecha_alta, usr_alta)");
            lStb.Append(" VALUES (" + id_modulo + ", '" + NombreModulo + "', " + id_proyecto + ", " + ID_Empresa + ", " + sec + ", " + (int)Enums.EstadoActivo.Activo + " , getdate(), " + usr_alta + ") ");
        ...
        public DataTable Obtener(int id_grupo = 0)
        {
            ...
            lStbProyectos.Append("SELECT g.id_modulo, g.nombreModulo, g.id_proyecto, g.id_empresa, p.Cod_proyecto, p.Nomb_Proyecto, e.nombre nombEmpresa ");
            lStbProyectos.Append(" FROM modulo g ");
            ...
            {
                DtbFases = null;
                MensajeError = "Error al Cargar los Modulos";
            }
